...
from skyrim.domain.character.queries import get_character_from_place, get_character_union_filters_by_user, name_exist
...
def create_player_view(request):
    if request.method=='POST':
        ...
        if(name == None or len(name) == 0 or name[0] == ""):
            is_valid = False
            messages.info(request,'Invalid name')

        if(is_valid or  name_exist(name)):
            is_valid = False
        ...
        if(known_spell_list == None or len(known_spell_list) == 0 or  known_spell_list[0] == ''):
            is_valid = False
            messages.info(request,"You must select some spells")
        
        race = request.POST.get('race_id',None)
        if race == None or len(race) == 0 or race[0] == "":
            is_valid = False
            messages.info(request,"Invalid race")
        if is_valid:
...
def create_beast_view(request):
    ...
    if request.method=='POST':
        ...
        if(place_id_list == None or len(place_id_list) == 0 or  place_id_list[0] == ''):
            messages.info(request,"You must select some places")
            is_valid = False
        ...
        blow = request.POST.get('blow',None)
        if blow == None or len(blow) == 0 or blow[0] == "":
            is_valid = False
            messages.info(request,"Invalid blow")
